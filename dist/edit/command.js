...
exports.withParamHandler = withParamHandler;
...
var Command = exports.Command = function () {
  ...
  _createClass(Command, [{
    ...
    value: function exec(pm, params) {
      ...
      var fromCx = contextParamHandler;
      var handler = fromCx || pm.options.commandParamHandler || defaultParamHandler;
      if (!handler) return false;
      handler(pm, this, function (params) {
        if (params) {
          if (fromCx) withParamHandler(fromCx, run.bind.apply(run, [_this.self, pm].concat(_toConsumableArray(params))));else run.call.apply(run, [_this.self, pm].concat(_toConsumableArray(params)));
        }
      });
    }
...
function withParamHandler(handler, f) {
  var prev = contextParamHandler;
  contextParamHandler = handler;
  try {
    return f();
  } finally {
    contextParamHandler = prev;
  }
}
